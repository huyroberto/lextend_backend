module.exports = (function () {
    ...
    router.route('/logout')
        .post(function (req, res) {
            ...
            console.log('Logout');
            backendService.getClient().logout(auth_token, function (err, response) {
                if (err) {
    ...
    router.route('/internal')
        .post(function (req, res) {
            try {
                ...
                authService.getClient().loginWithPassword(username, password, app_id, function (err, response) {
                    if (err) {
    ...
    router.route('/google')
        .post(function (req, res) {
            try {
                ...
                backendService.loginWithGoogleAccount(google_token, app_id, function (err, response) {
                    if (err) {
                        res.json({ response_code: '96', response_message: 'System Errors', data: err });
                        return;
                    }
    ...
    router.route('/forgot_password')
        .post(function (req, res) {
            ...
            backendService.getClient().forgotPassword(username, function (err, response) {
                if (err) {
                    res.json({ response_code: '96', response_message: 'System Errors', data: err });
                }
                if (response) {
